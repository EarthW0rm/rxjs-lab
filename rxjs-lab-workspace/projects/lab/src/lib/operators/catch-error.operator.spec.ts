...
describe('Operador: catchError', () => {
	...
	it('Utilizando o método catchError para caputurar o erro genérico e retornar um envelope error jsend', done => {
		TestBed.configureTestingModule({
			imports: [SwapiModule.forRoot('https://swapi.co/api-fake/')]
		});
		...
		const subscription$ = filmsService
			.GetFilms()
			.pipe(
				map((_: PagedResults<Film>) => {
					return { data: _, status: 'success' } as JsendEnvelope<
						PagedResults<Film>
					>;
				}),
				catchError(_ =>
					of({
						...
						status: 'error',
						message: _.message
					} as JsendEnvelope<PagedResults<Film>>)
				)
			)
			.subscribe(
				next => {
